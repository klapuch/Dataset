...
final class SqlRestSort extends Tester\TestCase {
	...
	public function testMinusAsDescend() {
		Assert::same(
			'ORDER BY foo desc',
			(new Dataset\SqlRestSort('-foo', ['foo']))->expression('')
		);
	}
	...
	public function testCombiningAllOperators() {
		Assert::same(
			...
			(new Dataset\SqlRestSort('foo,+bar,-baz', ['foo', 'bar', 'baz']))->expression('')
		);
	}
	...
	public function testAcceptingFirstPassedValue() {
		Assert::same(
			'ORDER BY foo asc',
			(new Dataset\SqlRestSort('foo,+foo,-foo', ['foo']))->expression('')
		);
	}
	...
	public function testMergingWithSource() {
		Assert::same(
			...
			(new Dataset\SqlRestSort('foo,+bar,-baz', ['foo', 'bar', 'baz']))->expression('prev')
		);
	}
	 ...
	 */
	public function testThrowingOnUnknownCriteria() {
		(new Dataset\SqlRestSort('foo,+bar,-baz', ['baz']))->expression('prev');
	}
}
